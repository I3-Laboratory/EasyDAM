...
def create_modules(module_defs):
    ...
    for i, module_def in enumerate(module_defs):
        ...
        if module_def['type'] == 'convolutional':
            ...
            if kernel_size == 2 and stride == 1:  #yolo-v3 tiny
            ...
            # fp_size = module_def['size']
            channels = module_def['filters']
            # se_module = SE_module(channels)
            se_module = SE_module(channels)
            modules.add_module('se_%d' % i, se_module)

        elif module_def['type'] == 'asff':
            filters = int(module_def['filters'])
            level = int(module_def['level'])
            asff_module = ASFF(level)
            modules.add_module('asff_%d' % i, asff_module)

        elif module_def['type'] == 'rfb':
            in_planes = int(module_def['in_planes'])
            out_planes = in_planes
            # out_planes = int(module_def['out_planes'])
            RFB_module = BasicRFB_a(in_planes,out_planes)  #输入feature map：in planes  输出feature map：out-planes
            modules.add_module('shortcut_%d' % i, RFB_module)

        elif module_def['type'] == 'yolo':
            yolo_index += 1
            stride = [32, 16, 8, 4, 2][yolo_index]
            layers = module_def['from'] if 'from' in module_def else []
            ...
            yolo_layer = YOLOLayer(anchors, nC, img_size, yolo_layer_count, hyperparams['cfg'], yolo_index, layers, stride)
...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, nC, img_size, yolo_layer, cfg, yolo_index, layers, stride):
    ...
    def forward(self, p, img_size, targets=None, var=None):
        ...
        if targets is not None:
            ...
            # Loss
            MSELoss = nn.MSELoss()  #position: xy wh
            BCEWithLogitsLoss = nn.BCEWithLogitsLoss()  #conf
            CrossEntropyLoss = nn.CrossEntropyLoss()    #class
            # CrossEntropyLoss = nn.BCEWithLogitsLoss()  # class
...
class Darknet(nn.Module):
    ...
    def forward(self, x, targets=None, var=0):
        ...
        img_size = x.shape[-1]
        layer_outputs = []  #store every layer feature map
        output = []         #store pred-layer feature map


        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            mtype = module_def['type']
            if mtype in ['convolutional', 'upsample', 'maxpool', 'rfb', 'se']:
                x = module(x)
            elif mtype == 'route':
                ...
                layer_i = [int(i) for i in module_def['from'].split(',')]
                for index, i in enumerate(layer_i):
                    if index == 0:
                        level_0 = layer_outputs[i]
                    elif index == 1:
                        level_1 = layer_outputs[i]
                    elif index == 2:
                        level_2 = layer_outputs[i]
                # print(level_0.size(),level_1.size(),level_2.size())
                x = module[0](level_0, level_1, level_2)
                # print(x.size())
            elif mtype =='yolo':
        ...
        # return self.losses if is_training else torch.cat(output, 1)
